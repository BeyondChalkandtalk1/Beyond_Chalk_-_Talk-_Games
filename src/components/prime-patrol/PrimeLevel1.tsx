...
//   // Add this useMemo near your other memos
//   const cellSize = useMemo(() => {
//     const maxGridPx = Math.min(window.innerWidth * 0.88, gridSize * 48);
//     return Math.floor(maxGridPx / gridSize);
//   }, [gridSize]);
//   const remaining = useMemo(
//     () =>
//       allPairs.filter(
//         ([a, b]) => !completed.some(([ca, cb]) => ca === a && cb === b),
//       ),
//     [allPairs, completed],
//   );
...
//     } else {
//       setNoRectMsg(
//         'Not a perfect rectangle! Clear and try again, or click "It\'s Prime!" if no rectangle is possible! 🤔',
//       );
//     }
//   }, [cells.size, phase]);
...
//         <div
//           ref={gridRef}
//           className="inline-grid p-2 bg-white/70 rounded-2xl shadow-lg"
//           style={{
//             gridTemplateColumns: `repeat(${gridSize}, ${cellSize}px)`,
//             gap: "2px",
//             width: `${gridSize * cellSize + (gridSize - 1) * 2 + 18}px`,
//             maxWidth: "95vw",
//           }}
...
//             const isDragSource = dragFrom === key;
...
//                   setIsDragging(true);
//                   // Pehle click pe decide karo — fill karna hai ya hatana hai
//                   if (isSelected) {
//                     setDragMode("remove");
//                     handleCellClick(r, c);
//                   } else {
...
//                 // onPointerEnter={() => {
//                 //   if (!isDragging || phase !== "playing") return;
//                 //   // Drag mode ke hisaab se fill ya remove karo
//                 //   setCells((prev) => {
//                 //     const next = new Set(prev);
//                 //     if (
//                 //       dragMode === "add" &&
...
//   // Grid size — non-trivial pairs se calculate karo, prime ke liye sqrt use karo
//   const gridSize = useMemo(() => {
//     const nonTrivial = allPairs.filter(([a]) => a > 1);

//     let maxDim = 6;
//     if (nonTrivial.length > 0) {
//       nonTrivial.forEach(([a, b]) => {
//         maxDim = Math.max(maxDim, a, b);
//       });
//     } else {
//       // Prime ya sirf 1×N — sqrt se reasonable grid
//       maxDim = Math.ceil(Math.sqrt(luckyNumber)) + 1;
//     }

//     return Math.min(Math.max(maxDim + 2, 8), 14); // max 14×14
//   }, [allPairs, luckyNumber]);

//   // Cell size — minimum 38px guarantee
//   const cellSize = useMemo(() => {
//     const available = Math.min(window.innerWidth * 0.92, 520);
//     return Math.max(Math.floor(available / gridSize), 38);
//   }, [gridSize]);
...
//       }, 2000);
//     } else {
//       // No rectangle — wrong cells highlight karo
...
//         setWrongCells(new Set());
//         if (primeNum) {
//           // Prime number — direct prime congrats
//           playSound(correctSound);
//           setPhase("primeCongrats");
//         } else {
//           // Composite hai lekin galat arrangement
//           playSound(incorrectSound);
//           setNoRectMsg(
...
//         <div
//           style={{ maxWidth: "95vw", maxHeight: "65vh", overflow: "auto" }}
...
//               gridTemplateColumns: `repeat(${gridSize}, ${cellSize}px)`,
//               gap: "2px",
//               width: `${gridSize * cellSize + (gridSize - 1) * 2 + 16}px`,
//               touchAction: "none",
//             }}
//             onPointerLeave={() => setIsDragging(false)}
//             onPointerUp={() => setIsDragging(false)}
...
export default function PrimeLevel1({
  ...
  // ─── KEY FIX: Alag gridRows aur gridCols ──────────────────────────────────
  // allPairs = [[1,66],[2,33],[3,22],[6,11]] for 66
  // gridRows = max(a values) + 2  =>  6 + 2 = 8   (rows kam chahiye)
  // gridCols = max(b values) + 2  =>  66 + 2 = 68  (cols zyada chahiye, scroll karega)
  const { gridRows, gridCols } = useMemo(() => {
    if (allPairs.length === 0) {
      // Pure prime — sqrt se chhoti grid
      const sq = Math.ceil(Math.sqrt(luckyNumber)) + 2;
      return { gridRows: Math.min(sq, 12), gridCols: Math.min(sq, 12) };
    }
    const maxA = Math.max(...allPairs.map(([a]) => a)); // sabse bada row value
    const maxB = Math.max(...allPairs.map(([, b]) => b)); // sabse bada col value
    return {
      gridRows: Math.min(maxA + 2, 20), // rows pe cap 20
      gridCols: Math.min(maxB + 2, 72), // cols pe cap 72 (1×66 ke liye enough)
    };
  }, [allPairs, luckyNumber]);

  // ─── Cell size: min 34px, screen se calculate ─────────────────────────────
  const cellSize = useMemo(() => {
