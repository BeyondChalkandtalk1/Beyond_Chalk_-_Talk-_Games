...
const PahalPractice = ({ onBack }: Props) => {
  const questions = useMemo(() => getPracticeQuestions(), []);
  ...
  const handleDrop = (item: DragOption) => {
    if (showFeedback) return;
    setDroppedItem(item);
    const isCorrect = item.label === question.correctDragLabel;
    const newAnswers = [...answers];
    newAnswers[currentQ] = item.label;
    setAnswers(newAnswers);
      ...
      <div className="max-w-2xl mx-auto">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="bg-card rounded-xl border border-border p-6 md:p-8 game-card-shadow"
          >
            ...
            {showFeedback && (
              <motion.div
                ...
                className={`mt-4 p-3 rounded-lg text-center font-display font-semibold ${
                  isCorrect
                    ? "bg-game-done/10 text-game-done"
                    : "bg-destructive/10 text-destructive"
                }`}
